function code = tAlternating_phase02
...
vr.session = struct(...
    ...
    'mazeName','tAlternating_phase01',... % change this to get the actual maze name from vr
...
elseif vr.fun.euclideanDist(vr.position(1:2),vr.trial(vr.tN).correctTarget)<vr.trial(vr.tN).correctRadius
    ...
    if ~vr.debugMode
        vr = giveReward(vr,vr.trial(vr.tN).rewardFraction);
...
if vr.mazeEnded
    ...
    if rewardsPerMinute > vr.session.targetRPM
        ...
        vr.trial(vr.tN+1).itiBlackout = min(10, vr.trial(vr.tN).itiBlackout + 0.1);
    else
        % make the maze easier by decreasing the ITI
        vr.trial(vr.tN+1).itiFrozen = 2;
        vr.trial(vr.tN+1).itiBlackout = max(0, vr.trial(vr.tN).itiBlackout - 0.1);
    end
