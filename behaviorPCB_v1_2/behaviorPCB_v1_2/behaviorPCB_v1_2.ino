...
unsigned long sendEveryMicros = 5000;
...
const int sync1Pin = 21;
const int sync2Pin = 20;
...
unsigned int sync1State = 0; // these are really bools, casting as ints for simplicity, may change later
unsigned int sync2State = 0; 
...
volatile long lickCount1 = 0;
volatile long lickCount2 = 0;
...
unsigned long syncEveryMilli = 1000; // every 10 seconds
...
// set up serial and print so that matlab knows it is working
  ...
  pinMode(sync1Pin,OUTPUT);
  pinMode(sync2Pin,OUTPUT);

  pinMode(lick1Pin,INPUT_PULLUP);
  pinMode(lick2Pin,INPUT_PULLUP);
  
  attachInterrupt(digitalPinToInterrupt(lick1Pin), updateLick1, RISING);
  attachInterrupt(digitalPinToInterrupt(lick2Pin), updateLick2, RISING);
...
void interpretCommand(String message) {
  ...
  String dataString = "dp,"+String(dP,3)+",dr,"+String(dR,3)+",dy,"+String(dY,3)+",l1,"+String(lickCount1)+",l2,"+String(lickCount2)+",v1,"+String(valve1State)+",v2,"+String(valve2State)+",s1,"+String(sync1State)+",s2,"+String(sync2State)+",dta,"+String(dt)+",dtmsg,"+String(millis()-lastMsgTime);
...
if (((millis()-lastSyncTime)>syncEveryMilli)||((millis()-lastSyncTime)<1)) {
  // change sync output
  sync1State = (int) random(0,2);
  sync2State = (int) random(0,2);
  digitalWrite(sync1Pin,sync1State);
  digitalWrite(sync2Pin,sync2State);
  lastSyncTime = millis();
}
 
//while((micros()-absTime)<sendEveryMicros) {
//  // do nothing
//}
//interpretCommand("000000");
